...
class GeneralGraphDataset(RecBoleDataset):
    def __init__(self, config):
        ...
        if not hasattr(self, 'user_positive_item_dict'):
            self.logger.info(set_color('Attribute "user_positive_item_dict" not found, creating it manually.', 'yellow'))
            # Use the efficient groupby method from RecBole to build the dictionary
            positive_items = self.inter_feat.groupby(self.uid_field)[self.iid_field]
            self.user_positive_item_dict = positive_items.apply(list).to_dict()
