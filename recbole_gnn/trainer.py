...
from .sampler import ICPSampler
import torch.cuda.amp as amp
from recbole.data.interaction import Interaction
from .utils import calculate_score

class ICPNSTrainer(Trainer):
    def __init__(self, config, model):
        ...
        self.stopping_step_ft = config["stopping_step_ft"]
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=True, callback_fn=None):
        ...
        if self.pretrain:
            ...
            self.logger.info(set_color('Pretraining Finished.', 'green'))
        
        if self.finetune:
            if self.config['strategy'].lower() == 'icpns':
                try:
                    self.resume_checkpoint(self.checkpoint_path)
                except:
                    ValueError("Checkpoint not found")
                self.logger.info("Stage 2: Finetuning")
            
            # Initialize sampler
            sampler = ICPSampler(self.config, train_data, self.model)
            self.sampler = sampler
            self.sampler.prepare_data()
            
            # Reset state
            self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.finetune_lr)
            self.logger.info(f"Resumed training from: {self.checkpoint_dir}")
            self.cur_step = 0
            self.best_valid_score=-np.inf
            
            epoch_avg_neg_scores = []
            
            for epoch_idx in range(self.finetune_epochs):
                self.model.train()
                total_loss = 0
                iter_data = tqdm(train_data, total=len(train_data), ncols=100, desc=set_color(f"Fine-tune {epoch_idx+1}", 'pink'))
                batch_count = 0
                training_start_time = time()
                
                for batch_idx, interaction in enumerate(iter_data):
                    interaction = interaction.to(self.device)
                    users = interaction[self.model.USER_ID]
                    neg_items = self.sampler.sample(users)
                    interaction[self.model.NEG_ITEM_ID] = neg_items
                ...
                self.logger.info(f"Epoch {epoch_idx+1}, Loss: {avg_loss}, Time:{training_end_time-training_start_time}")

                if epoch_idx % 1 == 0:
                    valid_score, valid_result = self._valid_epoch(valid_data, show_progress=show_progress)
                    self.best_valid_score, self.cur_step, stop_flag, update_flag = early_stopping(
                        valid_score, self.best_valid_score, self.cur_step,
                        max_step=self.stopping_step_ft, bigger=self.valid_metric_bigger
                    )
                    if verbose:
                        self.logger.info(set_color(f"Validating Epoch {epoch_idx+1}/{self.finetune_epochs}", 'green'))
                        self.logger.info(set_color('valid result', 'blue') + ': \n' + dict2str(valid_result))
                    if update_flag:
                        if saved:
                            self._save_checkpoint(epoch_idx)
                            self.logger.info(set_color('Saving current best (finetune)', 'blue') + f': {self.checkpoint_dir}')
                        self.best_valid_result = valid_result
                    if stop_flag:
                        self.logger.info(set_color(f'Early stopping at fine-tune epoch {epoch_idx}.', 'yellow'))
                        break
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if self.eval_step <= 0 or not valid_data:
                if saved:
                    self._save_checkpoint(epoch_idx)
                    update_output = set_color('Saving current', 'blue') + ': %s' % self.checkpoint_path
                    if verbose:
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if update_flag:
                    if saved:
                        self._save_checkpoint(epoch_idx)
                        update_output = set_color('Saving current best', 'blue') + ': %s' % self.checkpoint_path
                        if verbose:
                            self.logger.info(update_output)
                    self.best_valid_result = valid_result
