...
def get_trainer(model_type, model_name):
    ...
    """        
    try:
        return getattr(importlib.import_module('recbole_gnn.trainer'), 'ICPNSTrainer')
    except AttributeError:
        logger = getLogger()
        logger.info(
            set_color('Warning: ', 'red') + set_color('ICPNSTrainer Not Found, using default Trainer', 'red')
        )
...
def calculate_score(model, interaction):
